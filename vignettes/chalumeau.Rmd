...
mapview(plot_location_res, zcol = "partition_ssi_09")
mapview(plot_location_res, zcol = "partition_cal_02")
mapview(plot_location_res, zcol = "partition_cal_09")
```
...
plot_location_res2 <- plot_location |>
  ...
  mutate(partition_09 = res2$partition[, 9])
...
Dans les données de l'étude, les situaion aussi et seulement le niveau 0 est utilisé voici les valeurs: 
...
2. Un K-means (classification non supervisée) utilisant les 10 premiers axes de la RDA qui représente (`r round(sum(summary(res$rda)$cont$importance[2, 1:10])*100, 2)`% de l’inertie totale).
`), avec une augmentation progressive du nombre de groupe de 2 à 30 groupes, les indices utilisés sont:
