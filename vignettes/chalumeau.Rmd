...
```{R plot_rda, fig.width = 8, fig.height = 8}
triplot_rda(res$rda)
plot(res$km_ssi)
plot(res$km_cal)
plot(res$km_sil)
...
```{R maps, fig.height = 12, fig.width = 9, warning = FALSE}
...
plot_location_res <- plot_location |>
  ...
  mutate(partition_ssi_09 = res$km_ssi$partition[, 8]) |>
  mutate(partition_cal_02 = res$km_cal$partition[, 1]) |>
  mutate(partition_cal_09 = res$km_cal$partition[, 8]) |>
  inner_join(res$dat$full |> select(all_of(nm_var_cle)))

plot_grp_stat <- plot_location_res |>
  sf::st_drop_geometry() |>
  group_by(partition_ssi_09) |>
  summarise_at(all_of(nm_var), mean)

par(mfrow = c(3, 3))
for (i in 1:9) {
  tmp <- plot_grp_stat |>
    dplyr::filter(partition_ssi_09 == i) |>
    select(-partition_ssi_09) |>
    as.vector() |>
    unlist() |>
    sort() |>
    rev()
  plot(tmp[1:20],
    ...
    ylab = "Recouvrement moyen", xlab = "", main = paste0("Groupe ", i)
  )
  axis(2, las = 2)
  axis(1, at = 1:20, labels = names(tmp[1:20]), las = 2)
  box(bty = "l")
}
...
for (i in 1:9) {
  plot(domain_bio |> st_geometry(), 
    main = paste0("Groupe ", i))
  plot(
    plot_location_res_tr |>
      filter(partition_ssi_09 == i) |>
      st_geometry(), 
    add = TRUE,
    pch = 19,
    col = "steelblue",
...
La classification obtenue ne ressemble pas vraiment à ce qui est présenté dans le manuscript de Chalumeau et al. 

```{R map_chalumeau, fig.width = 6}
knitr::include_graphics("img/map_chalumeau.png")
```


Alors qu'un partage sur les données de précipitations et de températures fait bien ressortir des groupes spatialisés. 

```{R maps02, fig.width = 9, fig.height = 8}
res2 <- cascadeKM(
...
plot_location_res2 <- plot_location |>
  ...
  mutate(partition_09 = res2$partition[, 8])
mapview(plot_location_res2, zcol = "partition_02")
mapview(plot_location_res2, zcol = "partition_09")
```


## Détails des groupes issus de l'analyse de Chalumeau et al.

<!-- res$km_cal$partition[,8] |> table()
mapview(
    plot_location_res |> filter(partition_cal_09 == 1), 
    zcol = "partition_09"
  ) -->



# Remarque sur la réplication de l'étude

Dans les données de l'étude, les valeurs de situation aussi et seulement le niveau 0 est utilisé voici les valeurs: 
...
Pour résumer l'approche finale retenue dans les travaux d'Aurélie Chalumeau:

1. RDA (voir https://r.qcbs.ca/workshop10/book-en/redundancy-analysis.html) avec R^2^-adjusted de 14.63%:
...
2. Un K-means (classification non supervisée) utilisant les 10 premiers axes de la RDA qui représente (`r round(sum(summary(res$rda)$cont$importance[2, 1:10])*100, 2)`% de l’inertie totale), avec une augmentation progressive du nombre de groupes de 2 à 30, les indices utilisés sont:
  - *simple structure index* (ssi),
  - *average silhouette width* (silhouette),
  - *total within sum of square* (wss, aussi Calinkski)
La méthode des coudes appliquée sur l'indice wss identifie 9 groupes, mais ce sont les groupes basés sur l'indices de ssi qui sont utilisés (ce qui est problématique).
